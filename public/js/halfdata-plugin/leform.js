...
function replaceGETVariables(value) {
	...
	const replacedValue = Object
		...
		.reduce((newValue, [key, value]) => newValue.replaceAll(`{{${key}}}`, value), value);
	console.log(getVariables, replacedValue, jwtVariables)
	return replacedValue;
}
...
function leform_input_changed(_object) {
	...
	switch (type) {
		...
		case 'file':
			...
			leform_html_external_source_replace(element_id, var_value.join(', '));
			break;
		case 'checkbox':
		case 'imageselect':
		case 'tile':
		...
		switch (expressionType) {
			...
			case "row": {
				...
				rowValues.forEach((rowValue, rowIndex) => {
					...
					const formattedValue = rowValue.toFixed(decimalDigits)
